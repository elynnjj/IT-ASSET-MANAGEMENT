            ...
            if (progressId) {
                ...
                if (progressBar && progressBarFill && progressText && progressCount && statusMessage && statusMessageText) {
                    ...
                    let pollInterval = setInterval(function() {
                        fetch('{{ route("itdept.manage-users.import.progress") }}?progressId=' + progressId)
                            ...
                            .then(data => {
                                ...
                                let statusText = 'Processing import... (' + created + ' user(s) added';
                                if (emailsSent > 0) {
                                    statusText += ', ' + emailsSent + ' email(s) sent';
                                }
                                statusText += ')';
                                if (remaining > 0) {
                                ...
                                if (data.isComplete) {
                                    ...
                                    if (data.message) {
                                        ...
                                        // Redirect after 3 seconds if redirectRole is provided
                                        if (data.redirectRole) {
                                            setTimeout(function() {
                                                const currentUrl = new URL(window.location.href);
                                                currentUrl.searchParams.delete('progressId');
                                                currentUrl.searchParams.set('role', data.redirectRole);
